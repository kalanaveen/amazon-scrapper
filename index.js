...
const returnScraperApiUrl = (apiKey) => `http://api.scraperapi.com?api_key=b63060dc2f7b5ea861cce254bb06e380&autoparse=true`;
...
app.get('/bestsellers/:searchQuery', async (req, res) => {
    const { searchQuery } = req.params;
    const { api_key } = req.query;
    
    try {
        const response = await request(`${returnScraperApiUrl(api_key)}&url=https://www.amazon.in/gp/bestsellers/${searchQuery}`);
